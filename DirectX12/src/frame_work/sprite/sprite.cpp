...
#include "command_list/sprite_command_list.h"
...
public:
	...
	static constexpr float png_to_size = 100.0f;
	/**
	* @brief 頂点情報
	*/
	struct Vertex2D
...
HRESULT Sprite::Init(const std::wstring& path)
{
	...
	pimpl->command_list = SpriteCommandList::getinstance()->GetCommandList();
	...
	if (FAILED(hr))return hr;
	hr = pimpl->bundle.Init(SpriteCommandList::getinstance()->GetPipeline());
	if (FAILED(hr))return hr;
	hr = pimpl->SetBundle();
	if (FAILED(hr))return hr;
...
* @return 成功したかどうか
*/
HRESULT Sprite::Draw(const Transform& transform)
{
	HRESULT hr = S_OK;

	hr = SpriteCommandList::getinstance()->BeginScene();
	if (FAILED(hr))return hr;
	hr = pimpl->UpdateTransform(transform);
	if (FAILED(hr))return hr;
	//テクスチャをセット
	pimpl->texture.SetTexture(pimpl->command_list);
	pimpl->command_list->ExecuteBundle(pimpl->bundle.GetCommandList().Get());
...
HRESULT Sprite::Impl::CreatePlane()
{
	...
	vb[0] = Vertex2D{ Float3{-texture_size_x / png_to_size / 2,-texture_size_y / png_to_size / 2,0.0f},Float2{0,1} };
	vb[1] = Vertex2D{ Float3{-texture_size_x / png_to_size / 2,+texture_size_y / png_to_size / 2,0.0f},Float2{0,0} };
	vb[2] = Vertex2D{ Float3{+texture_size_x / png_to_size / 2,-texture_size_y / png_to_size / 2,0.0f},Float2{1,1} };
	vb[3] = Vertex2D{ Float3{+texture_size_x / png_to_size / 2,+texture_size_y / png_to_size / 2,0.0f},Float2{1,0} };
...
HRESULT Sprite::Impl::UpdateTransform(const Transform& transform)
{
	...
	Matrix pos_mat = DirectX::XMMatrixTranslation(pos.x, pos.y, -static_cast<float>(order_in_layer));
...
{
	return { pimpl->texture.GetWidth() / pimpl->png_to_size,pimpl->texture.GetHeight() / pimpl->png_to_size };
}

int Sprite::GetOrderInLayer()const noexcept
{
	return pimpl->order_in_layer;
}

void Sprite::SetOrderInLayer(const int layer)noexcept
{
	pimpl->order_in_layer = layer;
}
