...
* @param b 判定するボタン
*/
bool GamepadInput::GetButtonDown(size_t index, BUTTON b)const noexcept
{
...
* @param b 判定するボタン
*/
bool GamepadInput::GetButton(size_t index, BUTTON b)const noexcept
{
	if (!pimpl->index_check(index))return false;
	return pimpl->controller[index].bConnected &&
		pimpl->current_button[index][static_cast<int>(b)];
...
* @param b 判定するボタン
*/
bool GamepadInput::GetButtonUp(size_t index, BUTTON b)const noexcept
{
	if (!pimpl->index_check(index))return false;
...
*/
float GamepadInput::GetTrigger(size_t index, const bool right)const noexcept
{
...
*/
float GamepadInput::GetStick(size_t index, const bool right, const bool x)const noexcept
{
	if (!pimpl->index_check(index))return 0;
	//つながっていなかったら0を返す
	if (!pimpl->controller[index].bConnected) { return 0; }
